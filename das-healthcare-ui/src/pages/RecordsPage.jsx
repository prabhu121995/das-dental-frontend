...
const COLUMNS_BY_TYPE = {
  login: [
    ...
    { header: "agent_id", accessorKey: "agent_id", sortType: "number" },
    { header: "login_time", accessorKey: "login_time", sortType: "datetime" },
    { header: "logout_time", accessorKey: "logout_time", sortType: "datetime" },
...
export default function RecordsPage({ type }) {
  ...
  const skipNextTransactionRefetch = useRef(false);
  const skipNextModmedRefetch = useRef(false);
  const skipNextFormSubmissionRefetch = useRef(false);
  const skipNextNextechRefetch = useRef(false);
  const skipNextRefusedRefetch = useRef(false);
  ...
  useEffect(() => {
    if (type === "transaction" && hasAppliedTransaction.current) {
      if (skipNextTransactionRefetch.current) {
        skipNextTransactionRefetch.current = false;
        return;
      }
    ...
    if (type === "modmed" && hasAppliedModmed.current) {
      if (skipNextModmedRefetch.current) {
        skipNextModmedRefetch.current = false;
        return;
      }
    ...
    if (type === "form-submission" && hasAppliedFormSubmission.current) {
      if (skipNextFormSubmissionRefetch.current) {
        skipNextFormSubmissionRefetch.current = false;
        return;
      }
    ...
    if (type === "nextech" && hasAppliedNextech.current) {
      if (skipNextNextechRefetch.current) {
        skipNextNextechRefetch.current = false;
        return;
      }
    ...
    if (type === "refused" && hasAppliedRefused.current) {
      if (skipNextRefusedRefetch.current) {
        skipNextRefusedRefetch.current = false;
        return;
      }
  ...
  const handleFilter = async () => {
    if (type === "login") {
      ...
      try {
        ...
        const list = Array.isArray(res?.data) ? res.data : [];
        setLoginRows(list);
        setLoginTotalRows(typeof res?.total_rows === "number" ? res.total_rows : list.length);
        setTablePage(1);
      } catch (err) {
        if (err.response?.status === 401) {
          localStorage.removeItem("token");
    ...
    if (type === "break") {
      ...
      try {
        ...
        const list = Array.isArray(res?.data) ? res.data : [];
        setBreakRows(list);
        setBreakTotalRows(typeof res?.total_rows === "number" ? res.total_rows : list.length);
        setTablePage(1);
      } catch (err) {
        if (err.response?.status === 401) {
          localStorage.removeItem("token");
    ...
    if (type === "time-on-status") {
      ...
      setLoading(true);
      try {
        const res = await getTimeOnStatus({
          start_date: dateFrom,
          end_date: dateTo,
          page: 1,
          page_size: 100,
        ...
        const list = Array.isArray(res?.data) ? res.data : [];
        setTimeOnStatusRows(list);
        setTimeOnStatusTotalRows(typeof res?.total_rows === "number" ? res.total_rows : list.length);
        setTablePage(1);
      } catch (err) {
        if (err.response?.status === 401) {
          localStorage.removeItem("token");
    ...
    if (type === "transaction") {
      ...
      skipNextTransactionRefetch.current = true;
      setFilterError("");
      setLoading(true);
      try {
        const pageSize = tablePageSize === 0 ? (transactionTotalRows ?? 100000) : tablePageSize;
    ...
    if (type === "modmed") {
      ...
      skipNextModmedRefetch.current = true;
      setFilterError("");
      setLoading(true);
      try {
        const pageSize = tablePageSize === 0 ? (modmedTotalRows ?? 100000) : tablePageSize;
    ...
    if (type === "form-submission") {
      ...
      skipNextFormSubmissionRefetch.current = true;
      setFilterError("");
      setLoading(true);
      try {
        const pageSize = tablePageSize === 0 ? (formSubmissionTotalRows ?? 100000) : tablePageSize;
    ...
    if (type === "refused") {
      ...
      skipNextRefusedRefetch.current = true;
      setFilterError("");
      setLoading(true);
      try {
        const pageSize = tablePageSize === 0 ? (refusedTotalRows ?? 100000) : tablePageSize;
    ...
    if (type === "nextech") {
      ...
      skipNextNextechRefetch.current = true;
      setFilterError("");
      setLoading(true);
      try {
        const pageSize = tablePageSize === 0 ? (nextechTotalRows ?? 100000) : tablePageSize;
          ...
          <div className="fixed right-0 top-0 z-50 flex h-full w-full max-w-md flex-col border-l border-slate-800 bg-slate-950 shadow-xl">
            <div className="flex items-center justify-between border-b border-slate-800 px-4 py-3">
              <h2 className="text-sm font-semibold text-slate-50">
                Edit login — {editRow.agent_name ?? editRow.agent ?? `ID ${editRow.id}`}
              </h2>
              <button
                type="button"
                onClick={closeEditOffcanvas}
      ...
      {type === "time-on-status" && editRow && (
        <>
          ...
          <div className="fixed right-0 top-0 z-50 flex h-full w-full max-w-md flex-col border-l border-slate-800 bg-slate-950 shadow-xl">
            <div className="flex items-center justify-between border-b border-slate-800 px-4 py-3">
              <h2 className="text-sm font-semibold text-slate-50">
                Edit time on status — {editRow.agent_name ?? editRow.Agent ?? `ID ${editRow.Id}`}
              </h2>
              <button
                type="button"
                onClick={closeEditOffcanvas}
