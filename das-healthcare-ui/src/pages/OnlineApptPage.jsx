...
export default function OnlineApptPage() {
  ...
  return (
    <div className="flex h-full min-h-0 flex-col gap-4">
      <div className={`rounded-2xl border px-4 py-2 ${theme.bar}`}>
        <h1 className={`text-sm font-semibold ${theme.text}`}>Online Appt</h1>
      </div>
      <div className={`rounded-2xl border px-4 py-4 ${theme.bar}`}>
        <p className={`text-xs ${theme.muted}`}>Online appointment content goes here.</p>
      </div>
    </div>
  );
}
